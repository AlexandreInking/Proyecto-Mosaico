...
export function neutralMapJson(document: MapDocument): string {
  ...
  return `${JSON.stringify({
    ...
    layers: document.layers.map((layer, order) => ({ id: layer.id, name: layer.name, kind: layer.kind, autotileSetId: layer.autotileSetId, order, visible: layer.visible, locked: layer.locked, opacity: layer.opacity, isFolder: !!layer.isFolder, parentId: layer.parentId, collapsed: !!layer.collapsed, tiles: [...layer.cells.entries()].map(([key, tile]) => { const [x, y] = key.split(',').map(Number); return { x: x!, y: y!, tilesetId: tile.tilesetId, tileId: tile.tileId, flipX: !!tile.flipX, flipY: !!tile.flipY, rotation: tile.rotation ?? 0, autotileSetId: tile.autotileSetId, autotileProfile: tile.autotileProfile, animationId: tile.animationId, animationFrame: tile.animationFrame, animationDurationMs: tile.animationDurationMs } }).sort((left, right) => left.y - right.y || left.x - right.x) })),
  }, null, 2)}\n`
}
