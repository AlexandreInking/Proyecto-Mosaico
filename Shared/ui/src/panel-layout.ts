...
const defaults: Record<PanelLayoutMode, PanelLayout> = {
  map: { inspectorWidth: 304, timelineWidth: 208, timelineHeight: 112, tilesetHeight: 280, layersHeight: 220, timelineVisible: false, layersVisible: true },
  pixel: { inspectorWidth: 304, timelineWidth: 208, timelineHeight: 112, tilesetHeight: 220, layersHeight: 220, timelineVisible: true, layersVisible: true },
}
...
export function usePanelLayout(mode: PanelLayoutMode) {
  ...
  const resize = (panel: 'inspector' | 'timeline' | 'tileset' | 'layers', axis: 'width' | 'height', event: ReactPointerEvent) => {
    event.preventDefault(); event.currentTarget.setPointerCapture?.(event.pointerId)
    const startX = event.clientX; const startY = event.clientY; const startWidth = panel === 'inspector' ? layout.inspectorWidth : layout.timelineWidth; const startHeight = panel === 'timeline' ? layout.timelineHeight : panel === 'tileset' ? layout.tilesetHeight : layout.layersHeight
    const move = (next: PointerEvent) => {
      if (axis === 'width') {
        const width = clamp(startWidth + (panel === 'inspector' ? -(next.clientX - startX) : next.clientX - startX), 160, Math.max(220, window.innerWidth - 400))
        update(panel === 'inspector' ? { inspectorWidth: width } : { timelineWidth: width })
      }
  ...
  const applyLayout = (name: string) => { const next = saved[name]; if (next) setLayout({ ...defaults[mode], ...next }) }
  ...
  return { layout, saved, update, resize, saveLayout, applyLayout, deleteLayout, reset }
}
